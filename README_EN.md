...
One static binary for Windows/Linux/macOS — no runtime, no dependencies. Indexes large repositories in seconds, returns results to AI agents over MCP in milliseconds. 33 tools: 20 universal + 13 BSL-specific for 1C:Enterprise configurations.
...
- **33 MCP tools.** 20 universal (functions, classes, callers/callees, file content, grep) + 13 BSL-tools (object structure & profile, form handlers, event subscriptions, call graph, data links, register writers, role rights, impact map, data composition schema, read-only SQL).
...
| 1C:EDT metadata | quick-xml | `.mdo`, `.form`, `.rights` (v0.53.0), `.dcs` — composition schemas and template passports (v1.3.0) |
...
| `get_callers` | Who calls this function? **(v0.35.0)** each row carries the caller's source `path` (distinguishes same-named callers from different files). **v0.62.0:** for 1C configurations, form event handler bindings (`kind: form_binding`) are added to the code callers — such a procedure is run by the platform per an entry in the form description and has no call in the code; when the name belongs to several procedures, a single record with a sample `get_form_handlers` call arrives instead of a list. **v1.2.0:** for 1C configurations, qualified calls are found as well — `CommonModule.Method`, `Catalogs.Object.Method`, and `Variable.Method` from a form module when the method is exported from the object module of that form (a variable holding `FormAttributeToValue("Object")`); previously a bare name found only unqualified calls |
| `get_callees` | What does this function call? **(v0.35.0)** each row carries the source `path`. **v1.2.0:** in 1C, for a call with a receiver (`ОбработкаОбъект.Метод`, `CommonModule.Method`) `callee_path`/`callee_line` come from the call graph binding |
| `find_path` | **(v0.23.0)** Shortest path in the call graph between two functions `from`→`to` (iterative cycle-safe BFS over unique `calls` nodes, `max_depth=5`, any language). Returns path edges `[{caller, callee, line}]`. **v0.57.0:** an empty answer carries walk cut-off flags `walk_depth_exhausted` / `walk_nodes_capped`, and the hint distinguishes «hit the node cap», «ran out of depth» and «exhausted the reachable subgraph» — all three used to look like «no path». **v1.2.0:** in 1C the path continues through a call with a receiver — with the procedure the call graph bound it to |
| `get_call_tree` | **(v0.23.0)** Call tree from a `root` function up to `max_depth` (default 3). `direction`: `callees`/`down` (downstream) or `callers`/`up`. Flat edge list `[{caller, callee, line, depth, path}]` (**(v0.35.0)** `path` = source file of each edge) + nested `{name, children}` tree; `max_nodes` cap. **v1.2.0:** in 1C qualified calls are added upwards (as in `get_callers`), and downwards a call with a receiver continues with the bound procedure; tree nodes are procedure names. **v1.2.2:** the response is size-limited (48 KB by default): the farthest edges are dropped, there is no `tree` field, the response carries `truncated`, `edges_total`, `edges_shown` and a hint on how to get the rest; `max_response_bytes` — the budget for one call |
...
When BSL repos are present in `daemon.toml` (`language = "bsl"`), 13 BSL-specific tools are auto-registered:
...
| `get_dcs_schema` | **Data composition schema in one call (v1.4.0):** given a template name (`Report.MyReport.Template.Schema`), an owner name (`Report.MyReport`) or a common template (`CommonTemplate.Schema`) — the parsed schema: data sets with fields (name, presentation, type, role) and query texts, nested union sets, set links, calculated fields, totals, parameters, setting variants and counters. The `reads` section lists objects the schema reads in its query texts (`dcs_query` edges of the data-links graph). The data is parsed at index time into `dcs_schemas`/`dcs_datasets`, so reading `Template.xml` piece by piece is unnecessary. Parameters: `sections` for a narrow selection, `include_query=false` for query lengths only, `max_response_bytes` for a response budget (when space runs short, query texts go first, then set fields, then whole sections; identification and counters are never truncated). Both export formats: Designer and 1C:EDT |
...
Per-folder lifecycle: `not_started → initial_indexing → ready ⇄ reindexing_batch → (reindexing_extras →) ready / error`. `reindexing_extras` (1.7.0) means the batch core is committed and the 1C extras layer is still being computed. Each status transition is visible via `daemon status`.
...
**Default repository (1.5.0).** The `repo` parameter may be omitted: the server fills in the alias from `default_repo` under `[mcp]` in `daemon.toml`, or the only repository when there is just one. With several repositories and no default, a call without `repo` is rejected with the list of available aliases; the same list is shown in the parameter description in `tools/list`.
...
**Tool call time limit (1.7.0).** A tool call that does not finish within `tool_timeout_sec` seconds (`[mcp]` in `daemon.toml`, default 120) is aborted: the client gets an error suggesting a narrower query, and the database connection goes back to the pool. Otherwise a hung call would keep a read transaction open and the daemon could not checkpoint that database's WAL. Raise the limit on a slow machine with the database on a slow disk; `0` disables it.

**Waiting for a change batch (1.7.0).** While the daemon applies a batch of changes to a folder, serve no longer answers "Applying a batch of changes" right away: it waits for the batch to finish — up to `batch_wait_ms` milliseconds (`[mcp]` in `daemon.toml`, default 5000), polling the daemon every 25 ms — and answers with fresh data. The model does not have to spend a turn on a retry: a one-file batch takes milliseconds; in a 1C folder a module edit takes from a fraction of a second to a couple of seconds (measured on a stock trade configuration with 57k files: about 1.9 s, almost all of it the extras layer). If the batch is not done in time, the old answer is returned. Initial indexing (minutes) is not waited for. The wait happens before a database connection is taken and does not hold one. `0` disables waiting; raise the limit on a slow machine with 1C folders.

**Two-stage readiness (1.7.0).** In a folder with an extras layer (1C) a batch becomes ready in two steps. Right after the core — file text, procedures, calls — is committed, the daemon sets the folder to `reindexing_extras` and text tools answer without waiting: `read_file`, `get_function`, `get_class`, `grep_code`, `grep_body`, `grep_text`, `find_symbol`, `get_file_summary`, `list_files`, `stat_file`, `search_function`, `search_class`, `search_text`, `get_imports`. The rest — the call graph (`get_callers`, `get_callees`, `find_path`, `get_call_tree`) and all 1C tools — wait for the extras layer (same `batch_wait_ms` limit) and answer with the edit already reflected; if it is not done in time they refuse instead of silently returning a stale graph. Measured on a stock trade configuration (57k files): an edited module is visible to `read_file` and `get_function` in 0.06–0.12 s, the updated graph 1.8–1.9 s later. Folders without an extras layer behave as before. There is no setting — the behaviour is always on.

**Early readiness at startup (1.8.0).** When the daemon builds or checks a database directly on disk, the folder is declared ready right after the core, without waiting for the full-text index and the extras layer. While the full-text index is being built, `search_function`, `search_class` and `search_text` answer `{"status":"indexing","layer":"fts"}`; while the extras layer is being computed, extension (1C) tools answer `{"status":"indexing","layer":"extras"}` — retry the call later. All other tools work right away. A new database in the `auto` storage mode is usually built in memory by the daemon — such a folder opens all at once at the end of the pass. There is no setting.
