...
Один статический бинарник под Windows/Linux/macOS — без рантайма, без зависимостей. Индексирует крупные репозитории за секунды, отдаёт результат AI-агенту через MCP за миллисекунды. 33 инструмента: 20 универсальных + 13 для конфигураций 1С:Предприятие.
...
- **33 инструмента MCP.** 20 универсальных (функции, классы, вызывающие и вызываемые, содержимое файлов, поиск по коду) + 13 для 1С (структура и паспорт объекта, обработчики форм, подписки на события, граф вызовов, граф связей данных, регистраторы движений, права ролей, карта влияния, схема компоновки данных, произвольный SQL только на чтение).
...
При наличии BSL-репо в `daemon.toml` (`language = "bsl"`) автоматически добавляются 13 BSL-инструментов:
...
| `get_dcs_schema` | **Схема компоновки данных за один вызов (v1.4.0):** по имени макета (`Report.Отчёт.Template.Схема`), имени владельца (`Report.Отчёт`) или общего макета (`CommonTemplate.Схема`) — разобранная схема: наборы данных с полями (имя, представление, тип, роль) и текстами запросов, вложенные наборы объединения, связи наборов, вычисляемые поля, итоги, параметры, варианты настроек и счётчики. Секция `reads` — объекты, которые схема читает в текстах запросов (рёбра `dcs_query` графа связей данных). Данные разобраны при индексации в таблицы `dcs_schemas`/`dcs_datasets`, читать `Template.xml` по кускам не нужно. Параметры: `sections` — узкая выборка, `include_query=false` — только длины запросов, `max_response_bytes` — бюджет ответа (при нехватке места опускаются сначала тексты запросов, затем поля наборов, затем секции целиком; опознание и счётчики не урезаются). Оба формата выгрузки: Конфигуратор и 1C:EDT |
...
Жизненный цикл папки: `not_started → initial_indexing → ready ⇄ reindexing_batch → (reindexing_extras →) ready / error`. `reindexing_extras` (1.7.0) — ядро пачки уже записано, досчитывается надстройка 1С. Каждый переход виден через `daemon status`.
...
- `quick_window_ms` — окно одиночной правки (мс, по умолчанию 50): если за это окно после первого события не пришло событие по другому файлу, батч обрабатывается сразу, не дожидаясь `debounce_ms`; `0` — одиночный режим выключен
...
**Окно одиночной правки (`quick_window_ms`, 1.7.0).** Правка одного файла не ждёт `debounce_ms`: после первого события демон ждёт короткое окно (по умолчанию 50 мс). Нет за окно события по другому файлу — батч из одного файла обрабатывается сразу, правка видна в индексе примерно через 50–80 мс вместо ~0,55 с при `debounce_ms = 500`. Повторные события по тому же файлу (одно сохранение даёт 2–3 события) режим не переключают. Пришло событие по другому файлу — дальше прежний сбор: тишина `debounce_ms`, потолок `batch_ms`; массовые изменения (git pull, выгрузка конфигурации 1С) обрабатываются как раньше, отдельным батчем может уйти лишь первый файл. Задаётся для папки в `[[paths]]` файла `daemon.toml` или в `.code-index/config.json` проекта; `0` возвращает прежнее поведение. Новое значение для уже отслеживаемой папки применяется после перезапуска демона.
...
**Репозиторий по умолчанию (1.5.0).** Параметр `repo` можно не передавать: сервер подставит алиас из `default_repo` в секции `[mcp]` файла `daemon.toml`, а при единственном репозитории — его. Если репозиториев несколько и умолчание не задано, вызов без `repo` получает отказ со списком доступных алиасов; тот же список виден в описании параметра в `tools/list`.
...
**Предел времени вызова (1.7.0).** Вызов инструмента, не уложившийся в `tool_timeout_sec` секунд (секция `[mcp]` файла `daemon.toml`, по умолчанию 120), прерывается: клиент получает ошибку с подсказкой сузить запрос, соединение с базой возвращается в пул. Зависший вызов иначе держал бы открытое чтение базы, и демон не мог бы схлопнуть её журнал WAL. На слабой машине с базой на медленном диске предел можно поднять, `0` его выключает.

**Ожидание пачки изменений (1.7.0).** Пока демон применяет пачку изменений папки, serve не отказывает сразу ответом «Применяется батч изменений», а ждёт её окончания — до `batch_wait_ms` миллисекунд (секция `[mcp]` файла `daemon.toml`, по умолчанию 5000), опрашивая демон каждые 25 мс, — и отвечает свежими данными. Модели не нужно тратить ход на повтор: пачка одного файла обрабатывается за миллисекунды, в папке 1С правка модуля занимает от долей секунды до пары секунд (замер на типовой торговой конфигурации в 57 тыс. файлов — около 1,9 с, почти всё — надстройка). Не дождался — прежний ответ. Первичную индексацию (идёт минутами) serve не ждёт. Ожидание идёт до взятия соединения с базой и его не держит. `0` — не ждать; на медленной машине с папками 1С предел можно поднять.

**Готовность в две ступени (1.7.0).** В папке с надстройкой (1С) пачка готова в два шага. Сразу после записи ядра — текста файлов, процедур, вызовов — демон ставит папке статус `reindexing_extras`, и инструменты текста отвечают без ожидания: `read_file`, `get_function`, `get_class`, `grep_code`, `grep_body`, `grep_text`, `find_symbol`, `get_file_summary`, `list_files`, `stat_file`, `search_function`, `search_class`, `search_text`, `get_imports`. Остальные — граф вызовов (`get_callers`, `get_callees`, `find_path`, `get_call_tree`) и все инструменты 1С — ждут, пока досчитается надстройка (тот же предел `batch_wait_ms`), и отвечают уже с учётом правки; не дождались — отказ, устаревший граф молча не отдаётся. Замер на типовой торговой конфигурации (57 тыс. файлов): правленый модуль виден `read_file` и `get_function` через 0,06–0,12 с, граф с правкой — ещё через 1,8–1,9 с. Для папок без надстройки ничего не меняется. Настройки нет — поведение всегда включено.

**Досрочная готовность при старте (1.8.0).** Когда демон строит или сверяет базу прямо на диске, папка объявляется готовой сразу после ядра, не дожидаясь полнотекста и надстройки. Пока собирается полнотекст, `search_function`, `search_class` и `search_text` отвечают `{"status":"indexing","layer":"fts"}`; пока досчитывается надстройка, инструменты расширений (1С) отвечают `{"status":"indexing","layer":"extras"}` — вызов стоит повторить позже. Остальные инструменты работают сразу. Новую базу в режиме хранения `auto` демон обычно строит в оперативной памяти — такая папка открывается целиком в конце прохода. Настройки нет.

Без обёртки идут диагностические `health`, `get_stats`, `stat_file` — формат не менялся.

**Номер версии данных (1.4.1).** У базы есть номер того, как собраны её данные; он сверяется с номером, зашитым в бинарник. Номер выпуска тут ни при чём: он поднимается только когда правка меняет состав или разбор собираемых данных, поэтому смена версии сама по себе базу устаревшей не делает. Число записывается лишь после прохода, разобравшего каждый файл (`index <путь> --force` или сборка пустой базы) — обычное обновление по изменённым файлам его не поднимает, ведь остальные файлы разобраны прежним способом.

`get_stats` и `health` отдают блок `data_version` — чем собрана база, что в бинарнике, признак расхождения; при расхождении добавляются объяснение и готовая команда пересборки. Ответы BSL-инструментов, **закончившиеся ошибкой**, получают тот же набор в разделе `stale_index`; успешные ответы не меняются. Для папок удалённого узла решение принимается по его собственной базе. Пересборка при этом никогда не запускается сама — когда её делать, решает человек.

Подсказка не приходит на пустые ответы: пустой список сплошь и рядом законен, а отличить «данные не собраны» от «в конфигурации этого нет» без чтения исходных файлов нельзя. В таких случаях расхождение видно в `get_stats`.
