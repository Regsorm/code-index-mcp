...
* `target/release/bsl-indexer[.exe]` — с полной поддержкой 1С (XML-парсеры, граф вызовов BSL, граф связей данных, MCP-tools `get_object_structure`/`get_form_handlers`/`find_path`/`search_terms`/`get_data_links`/`find_data_path`/`get_register_writers` и опциональный LLM-enrichment под cargo feature `enrichment`).
...
`get_event_subscriptions`, `find_path`, `search_terms`, `get_data_links`,
