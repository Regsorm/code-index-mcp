...
| Метаданные 1С:EDT | quick-xml | `.mdo`, `.form`, `.rights` (v0.53.0), `.dcs` — схемы компоновки и паспорта макетов (v1.3.0) |
...
| `get_callers` | Кто вызывает данную функцию? **(v0.35.0)** каждая запись несёт `path` файла-вызывателя (различает одноимённых вызывателей из разных файлов). **v0.62.0:** для конфигураций 1С к вызывателям из кода добавляются привязки обработчиков форм (`kind: form_binding`) — процедуру выполняет платформа по записи в описании формы, вызова в коде у неё нет; когда имя носят несколько процедур, вместо списка приходит запись с образцом вызова `get_form_handlers`. **v1.2.0:** для конфигураций 1С находятся и вызовы с квалификатором — `ОбщийМодуль.Метод`, `Справочники.Объект.Метод` и `Переменная.Метод` из модуля формы, когда метод экспортно объявлен в модуле объекта этой формы (переменная с `РеквизитФормыВЗначение("Объект")`); прежде по голому имени находились только вызовы без квалификатора |
| `get_callees` | Что вызывает данная функция? **(v0.35.0)** каждая запись несёт `path` файла-источника. **v1.2.0:** для 1С у вызова с приёмником (`ОбработкаОбъект.Метод`, `ОбщийМодуль.Метод`) `callee_path`/`callee_line` берутся из привязки графа вызовов |
| `find_path` | **(v0.23.0)** Кратчайший путь в графе вызовов от функции `from` до `to` (итеративный cycle-safe BFS по уникальным узлам `calls`, `max_depth=5`, любой язык). Возвращает рёбра пути `[{caller, callee, line}]`. **v0.57.0:** пустой ответ несёт признаки обрыва обхода `walk_depth_exhausted` / `walk_nodes_capped`, а подсказка различает «упёрлись в потолок узлов», «не хватило глубины» и «достижимый подграф исчерпан целиком» — раньше все три случая выглядели как «пути нет». **v1.2.0:** для 1С путь продолжается через вызов с приёмником — процедурой, к которой его привязал граф вызовов |
| `get_call_tree` | **(v0.23.0)** Дерево вызовов от функции `root` на глубину `max_depth` (по умолчанию 3). `direction`: `callees`/`down` (вглубь) или `callers`/`up`. Плоский список рёбер `[{caller, callee, line, depth, path}]` (**(v0.35.0)** `path` = файл-источник ребра) + вложенное дерево `{name, children}`; cap `max_nodes`. **v1.2.0:** для 1С вверх добавляются вызовы с квалификатором (как в `get_callers`), вниз вызов с приёмником продолжается привязанной процедурой; узлы дерева — имена процедур. **v1.2.2:** ответ ограничен по размеру (по умолчанию 48 КБ): дальние рёбра отбрасываются, поля `tree` нет, в ответе `truncated`, `edges_total`, `edges_shown` и подсказка, как добрать остальное; `max_response_bytes` — бюджет на один вызов |
