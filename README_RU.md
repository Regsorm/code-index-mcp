...
`_meta.dependent_files` — список файлов, на которых построен ответ. Используется кэширующим прокси (`mcp-cache-ci` 0.2.0+) для точечной инвалидации записей кэша при изменении файла на диске. Клиенты, которые не используют это поле, просто читают `result` как раньше.
