...
Один статический бинарник под Windows/Linux/macOS — без рантайма, без зависимостей. Индексирует крупные репозитории за секунды, отдаёт результат AI-агенту через MCP за миллисекунды. 25 tools: 18 универсальных + 7 BSL-специфичных для конфигураций 1С:Предприятие.

## Что внутри

- **Производительность.** 62 000 файлов проиндексировано за 43 секунды, поиск <1 мс на запрос. Подходит для монорепо 100K+ файлов.
- **25 MCP tools.** 18 универсальных (функции, классы, callers/callees, content файлов, grep) + 7 BSL-tools (object structure, обработчики форм, подписки на события, граф вызовов, граф связей данных).
- **Native BSL/1С.** Парсит XML-выгрузки конфигураций 1С:Предприятие 8.3. Граф связей данных (рёбра объект→объект по ссылочным типам реквизитов) для типичной бухгалтерии — ~60 000 рёбер за пару секунд.
...
| `get_call_tree` | **(v0.23.0)** Дерево вызовов от функции `root` на глубину `max_depth` (по умолчанию 3). `direction`: `callees`/`down` (вглубь) или `callers`/`up`. Плоский список рёбер `[{caller, callee, line, depth}]` + вложенное дерево `{name, children}`; cap `max_nodes` |
...
| Инструмент | Описание |
|------------|----------|
| `get_object_structure` | Полная структура объекта конфигурации 1С по `full_name` (`Document.РеализацияТоваровУслуг`): реквизиты с типами в 1С-нотации, табличные части с колонками, измерения/ресурсы регистров; `enum_values` для перечислений; `predefined` для объектов с предопределёнными элементами (справочники, планы счетов); `posting` для документов (свойства проведения из корневого `<Properties>`: `Posting`, `RealTimePosting`, `RegisterRecordsDeletion`, `RegisterRecordsWritingOnPost` — только у документов). Базовые секции (`attributes`/`dimensions`/`resources`/`tabular_sections`) присутствуют всегда (пустые — как `[]`). Параметр `sections` (v0.29.0) — узкая выборка: вернуть только указанные секции (например `["posting"]` — ~0.2 КБ вместо полного объекта) |
...
| `get_event_subscriptions` | Все подписки на события из `EventSubscriptions/*.xml` с фильтром по handler-модулю. Имена событий нормализованы к русским (`OnWrite`→`ПриЗаписи`); фильтр принимает и русское имя, и английский enum платформы |
| `find_path_bsl` | Цепочка вызовов между двумя процедурами через `proc_call_graph` (recursive CTE, max_depth=3). BSL-вариант универсального `find_path` — `proc_call_graph` хранит `call_type` и ключи процедур |
...
`_meta.dependent_files` — список файлов, на которых построен ответ; `_meta.file_mtimes` (с 0.20.0) сопоставляет каждому из них индексный mtime (unix-секунды). Оба поля использует парный кэширующий прокси **`mcp-cache-ci`** (`dependent_files` — с его 0.2.0; `file_mtimes` — с 0.4.0+): для точечной инвалидации при изменении файла на диске и для write-triggered ленивой ревалидации — при FS-событии демон также шлёт ранний `POST /mark-dirty {repo, files:[{path, mtime}]}`, и прокси продолжает читать из источника, пока индекс не догонит диск (`index_mtime >= observed`), и только потом снова кэширует. Клиенты, которые не используют эти поля, просто читают `result` как раньше.
