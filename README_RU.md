...
**Как работает `auto`.** Перед индексацией папка взвешивается: складывается вес файлов, которые пойдут в индекс (обход только сведений о файлах, 1–3 секунды на 60 тысяч файлов). Ожидаемый расход памяти — вес × 3 (множитель по замеру: папка 3,98 ГБ дала 9,8 ГБ занятой памяти). Если он укладывается в разрешённую долю свободной памяти на этот момент, база собирается в оперативной памяти и затем сбрасывается на диск; иначе — сразу на диске. После работы в памяти освобождённое возвращается системе, поэтому следующая папка получает её обратно. Решение с числами пишется в журнал.
