...
fn process_batch(
    ...
    if commit_ok {
        if let Some(proc) = ctx.resolved_processor {
            if failed == 0 && done == batch_len {
                // COMMIT уже открыл читателям ядро, но граф пока ещё не пересчитан.
                tokio_block_on(async {
                    ctx.state
                        .set_status(ctx.path, PathStatus::ReindexingExtras)
                        .await;
                });
            }
...
///
/// Порядок важнее содержания: после COMMIT текстовые инструменты уже отвечают
/// по ядру, а инструменты надстройки ждут полной готовности. Схлопывание WAL
/// этого ожидания не стоит — данные видны сразу после COMMIT.
///
        ...
        if need_full && !worked_in_memory && !stop.load(Ordering::Acquire) {
            ...
            tracing::info!(
                "[{}] базовая индексация готова — папка объявлена доступной; \
                 ...
                 инструменты расширения включатся по её завершении",
                path.display(),
                proc.name()
            );
        }
