...
/// Порядок вызовов: `begin` один раз до первой порции, `flush` после записи
/// каждой порции, `write` один раз в самом конце.
pub trait ParseExtrasCollector: Send + Sync {
    ...
    /// Подготовить базу к приёму сырья: очистить слои, которые сборщик строит
    /// заново. Зовётся ОДИН раз, до первой порции. По умолчанию — ничего.
    fn begin(&self, _storage: &mut Storage) -> anyhow::Result<()> {
        Ok(())
    }

    /// Сбросить накопленное сырьё в базу и освободить буферы. Зовётся после
    /// записи КАЖДОЙ порции: иначе сборщик копил бы обращения и комментарии по
    /// всему дереву разом, и деление на порции не удержало бы расход памяти.
    /// По умолчанию — ничего.
    fn flush(&self, _storage: &mut Storage) -> anyhow::Result<()> {
        Ok(())
    }
...
pub trait LanguageProcessor: Send + Sync {
    ...
    /// Пересбор по-прежнему разовый и запускается вручную, но расхождение теперь
    /// видно снаружи через номер версии данных
    /// ([`crate::storage::Storage::data_version_status`]), и инструменты сообщают
    /// о нём сами (поле `stale_index` в ответе-ошибке). Поведение `extras_present`
    /// от этого не меняется.
    fn extras_present(&self, _storage: &Storage) -> bool {
        false
    }

    /// «Вызыватели» процедуры, которых нет и не может быть в графе вызовов
    /// кода: декларативные привязки из описаний. Процедура объявлена в модуле,
    /// но зовёт её платформа по записи в описании объекта (обработчик события
    /// формы и т.п.) — вызова в коде нет, и `get_callers` без этого хука
    /// отвечает «нигде не вызывается», хотя привязка есть.
    ///
    /// Возвращает готовые записи для ответа `get_callers`. У каждой обязателен
    /// ключ `caller` (кто зовёт, человекочитаемо) и `kind` (вид привязки);
    /// желателен `path` — файл описания, он уходит в `dependent_files`, чтобы
    /// кэш ответа сбрасывался при правке этого файла. Default — пусто:
    /// у универсальных языков декларативных привязок нет.
    ///
    /// Записей должно быть немного: процедура адресуется ИМЕНЕМ, и если имя
    /// принадлежит многим процедурам сразу, реализация обязана вернуть не
    /// длинный список, а одну запись о том, что имя неоднозначно.
