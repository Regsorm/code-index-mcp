...
pub struct IndexConfig {
    ...
    /// Бюджет одной порции полного разбора — сколько байт исходников читается,
    /// разбирается и пишется за один заход (по умолчанию 512 МБ).
    ///
    /// Пик расхода памяти определяется не размером базы, а этим числом:
    /// содержимое порции, разбор и сжатые копии живут одновременно, и на каждый
    /// байт исходников приходится примерно втрое больше памяти (тот же разброс,
    /// что у `memory_estimate_factor`). Порции идут одна за другой, память
    /// освобождается после каждой — поэтому расход перестаёт зависеть от того,
    /// сколько весит папка целиком.
    ///
    /// Уменьшать, когда памяти мало (контейнер с жёстким лимитом): порций станет
    /// больше, расход — меньше. Увеличивать, когда памяти много: меньше границ,
    /// на которых однопоточная запись не перекрывается разбором. Ноль отключает
    /// деление на порции — всё дерево читается разом, как до его появления.
    ///
    /// Работает только там, где в разбор идёт всё дерево: первичная индексация,
    /// принудительный полный разбор и продолжение прерванной загрузки. На обычных
    /// правках (наблюдатель, сверка при старте) файлов мало, и деление на порции
    /// ничего не меняет.
    ...
    ///
    /// По умолчанию 2 — это середина наблюдаемого разброса, а НЕ запас сверху.
    /// Замеры первичной индексации с базой в памяти: 1,4 ГБ исходников (сайт,
    /// 191 тыс. мелких файлов) → 2,4 ГБ израсходованной памяти (×1,7); 3,7 ГБ
    /// (типовая торговая конфигурация, 60 тыс. файлов) → 4,4 ГБ (×1,2); 5,3 ГБ
    /// (типовая учётная, 92 тыс. файлов) → 8,8 ГБ (×1,6). Разброс зависит от
    /// языка, плотности кода в файлах и состава папки, поэтому предсказать
    /// множитель заранее нельзя — он подбирается по журналу.
    ///
    /// Значение снижено с 3 до 2 вместе с появлением порционного разбора
    /// (`chunk_budget_bytes`): содержимое файлов и их разбор больше не лежат в
    /// памяти целиком, и на тех же папках расход упал примерно на треть.
    ///
    /// Когда увеличивать (3 и выше): памяти мало, машина слабая, и уйти на
    ...
    ///
    /// Когда уменьшать (1,3–1,7): памяти заведомо много и есть готовность
...
fn default_chunk_budget_bytes() -> usize {
    512 * 1_048_576 // 512 МБ исходников на порцию
}
...
mod tests {
    ...
    fn test_memory_estimate_default_factor() {
        let cfg = IndexConfig::default();
        assert_eq!(cfg.memory_estimate_factor, 2.0);
        // 1 ГБ исходников при множителе 2 — 2 ГБ ожидаемого расхода.
        assert_eq!(cfg.memory_estimate_bytes(1_000_000_000), 2_000_000_000);
    }
    ...
    fn test_memory_estimate_bad_factor_falls_back_to_default() {
        ...
        for bad in [0.0, -2.0, f32::NAN, f32::INFINITY] {
            ...
            assert_eq!(cfg.memory_estimate_factor_effective(), 2.0);
            assert_eq!(cfg.memory_estimate_bytes(1_000_000_000), 2_000_000_000);
        }
    }
