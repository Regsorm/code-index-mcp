# bsl-indexer / code-index v0.46.0
...
**Два бинарника:**
- `code-index` — публичный, поддерживает Python, Rust, Go, Java, JavaScript, TypeScript, PHP, C, C++, C#, Ruby, Swift, HTML + текстовые файлы.
- `bsl-indexer` — приватный, включает всё из `code-index` плюс 12 BSL-специфичных MCP-инструментов для конфигураций 1С:Предприятие.
...
XML-файл выгрузки 1С попадает в кодовый индекс, если распознан объект
метаданных (с 0.51.0 — все 45 типов верхнего уровня, включая роли,
определяемые типы, HTTP-сервисы и общие формы). По таким файлам работают
`grep_code`, `find_symbol`, `get_class`; `grep_text` их не видит. Прочий XML —
в том числе `Configuration.xml`, где объекта метаданных нет, — остаётся
текстовым, и ищется как раньше через `grep_text`/`search_text`.
...
| `memory_max_percent` | `50` | Какую долю СВОБОДНОЙ памяти разрешено занять под базу в auto-режиме |
| `memory_estimate_factor` | `2.0` | Во сколько раз работа в памяти дороже веса исходников (auto-режим) |
| `chunk_budget_bytes` | `536870912` (512 МБ) | Сколько байт исходников идёт в один заход полного разбора; `0` — без деления |
| `debounce_ms` | `1500` | Задержка watcher (мс) |
| `batch_ms` | `30000` | Максимальное время накопления батча (мс) |
| `bulk_batch_threshold` | `8000` | С какого числа изменившихся файлов пачку выгоднее обработать полным проходом |
...
| `get_callers` | Кто вызывает данную функцию (граф вызовов). Для конфигураций 1С — плюс привязки обработчиков форм: их выполняет платформа, вызова в коде нет; и вызовы с квалификатором: `ОбщийМодуль.Метод`, `Справочники.Объект.Метод`, вызов метода объекта из модуля его формы |
| `get_callees` | Что вызывает данная функция (граф вызовов). Для 1С у вызова с приёмником место определения берётся из привязки графа |
| `get_imports` | Импорты файла или модуля |
| `get_file_summary` | Полная карта файла: функции, классы, импорты, переменные |
| `find_path` | **(v0.23.0)** Кратчайший путь в графе вызовов между двумя функциями (BFS по `calls`, любой язык, `max_depth=5`). Для 1С путь продолжается через вызов с приёмником по привязке графа |
| `get_call_tree` | **(v0.23.0)** Дерево вызовов от функции вниз/вверх (`direction=callees\|callers`, `max_depth`, `max_nodes`). Для 1С вверх — с вызовами с квалификатором, вниз — по привязке графа. С v1.2.2 ответ ограничен по размеру (48 КБ по умолчанию): сокращённый — без `tree`, с `edges_total`/`edges_shown` и подсказкой, как добрать остальное |
...
| `grep_text` | Regex по содержимому текстовых файлов (.yaml, .md, .toml и др.; XML объектов 1С сюда НЕ входит — он кодовый) |
| `grep_code` | **(v0.8.0)** Regex по содержимому code-файлов через таблицу `file_contents` — включая XML объектов 1С |
...
| `get_role_rights` | **(v1.0.0)** Права ролей одним вызовом: по объекту — роли со списком прав каждой, по роли — объекты с правами. Источник — таблица `role_rights` |
...
### Бенчмарки (HDD, Windows, август 2026)

| Кодовая база | Файлов | Полная индексация | Запуск на готовом индексе |
|---|---|---|---|
| Управление Торговлей | 57 072 | 2 мин 41 с | 2,3 с |
| Бухгалтерия предприятия | 88 284 | 5 мин 51 с | 5,3 с |
| сайт на PHP | 157 772 | 13 мин 1 с | 8,0 с |

Полная индексация — разбор с нуля; у Бухгалтерии это 2 мин 38 с ядро, 1 мин 58 с надстройка 1С, 56 с сброс базы на диск. Повторный запуск использует `mtime + file_size` fast-path: только `stat()` на каждый файл, без чтения и без SHA-256.

### Время поиска

| Операция | Время |
|---|---|
| Поиск функции по имени | около 10 мс по HTTP, повторный из кэша — доли миллисекунды |
| Граф вызовов (get_callers/get_callees) | миллисекунды |
| FTS-поиск по 261K функций | около 10 мс по HTTP |
